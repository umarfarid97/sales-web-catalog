...
export const StoreProvider = ({ children }) => {
  ...
  const placeOrder = async (customerOrOrderData, paymentMethodParam, options = {}) => {
    ...
    // Generate guaranteed unique, collision-proof Order ID (e.g. ORD-260910-84729)
    // Date: YYMMDD (6 digits), Millisecond time component (5 digits), Entropy (2 digits). Total: 18 chars (fits < 20 gateway limit)
    const now = new Date();
    const datePart = now.toISOString().slice(2, 10).replace(/-/g, '');
    let orderId = '';
    let collisionAttempts = 0;
    do {
      const timeSlice = (Date.now() % 100000).toString().padStart(5, '0');
      const randSuffix = Math.floor(10 + Math.random() * 90);
      orderId = `ORD-${datePart}-${timeSlice}${randSuffix}`.slice(0, 19);
      collisionAttempts++;
    } while (orders.some((o) => o.id === orderId) && collisionAttempts < 10);
